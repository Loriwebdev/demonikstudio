...
class CodelessHeaderImporter{
	...
	public function ajax_handler(){
		...
		if( $_POST['action'] == 'cl_import_header_data' )
			$this->process_install_header();

		if( $_POST['action'] == 'cl_import_footer_data' )
	...
	public function process_install_header(){
		...
		wp_send_json_success( array('message' => 'Header Successfully imported') );
	}
	...
	public function process_install_footer(){
		...
		wp_send_json_success( array('message' => 'Footer Successfully imported') );
	}
	...
	// Read the file that will be written
		public function read_file_header($file, $dir){
			...
			if ( file_exists($file) ) {
				...
				$this->message = esc_html__("File doesn't exist", "june");
			}
...
class CodelessImporter{
	...
	public function process_import_options(){
		...
		if( $theme_mods && ! empty( $theme_mods ) ){
			...
			if( $options ){
				...
				update_option( 'woocommerce_cart_page_id', 38 );
				update_option( 'woocommerce_checkout_page_id', 39 );
				update_option( 'woocommerce_myaccount_page_id', 40 );
			}else{
				wp_send_json_error( array('message' => 'Options not loaded or files missed.') );
			}
		}
	...
	public function ajax_plugins() {
			...
			$json['active'] = array(
						...
						'action2' => -1,
						'message' => esc_html__( 'Activating Plugin','june' ),
			);
			...
			$json['install'] = array(
						...
						'action2' => -1,
						'message' => esc_html__( 'Installing Plugin','june' ),
			);
			...
			if ( $json && ( !empty($json['active']['plugin']) || !empty($json['install']['plugin']) ) ) {
				...
				wp_send_json_success( array( 'message' => esc_html__( 'No plugins to install or activate', 'june' ) ) );
			}
			exit;
